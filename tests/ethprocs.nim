...
import nimcrypto, asyncdispatch2, stint
import ../rpcserver, stintjson, ethtypes
...
rpc("web3_sha3") do(data: string) -> string:
...
rpc("eth_coinbase") do() -> string:
  ## Returns the current coinbase address.
  result = ""

rpc("eth_mining") do() -> bool:
  ## Returns true of the client is mining, otherwise false.
  discard

rpc("eth_hashrate") do() -> int:
  ## Returns the number of hashes per second that the node is mining with.
  discard

rpc("eth_gasPrice") do() -> int64:
  ## Returns an integer of the current gas price in wei.
  discard

rpc("eth_accounts") do() -> seq[array[20, byte]]:
...
rpc("eth_blockNumber") do() -> int:
  ## Returns integer of the current block number the client is on.
  discard

rpc("eth_getBalance") do(data: array[20, byte], quantityTag: string) -> int:
...
rpc("eth_getStorageAt") do(data: array[20, byte], quantity: int, quantityTag: string) -> seq[byte]:
...
rpc("eth_getTransactionCount") do(data: array[20, byte], quantityTag: string):
...
rpc("eth_getBlockTransactionCountByHash") do(data: array[32, byte]) -> int:
...
rpc("eth_getBlockTransactionCountByNumber") do(quantityTag: string) -> int:
...
rpc("eth_getUncleCountByBlockHash") do(data: array[32, byte]):
...
rpc("eth_getUncleCountByBlockNumber") do(quantityTag: string):
...
rpc("eth_getCode") do(data: array[20, byte], quantityTag: string) -> seq[byte]:
...
rpc("eth_sign") do(data: array[20, byte], message: seq[byte]) -> seq[byte]:
...
rpc("eth_sendTransaction") do(obj: EthSend) -> UInt256:
...
rpc("eth_sendRawTransaction") do(data: string, quantityTag: int) -> UInt256: # TODO: string or array of byte?
...
rpc("eth_call") do(call: EthCall, quantityTag: string) -> UInt256:
...
rpc("eth_estimateGas") do(call: EthCall, quantityTag: string) -> UInt256: # TODO: Int or U/Int256?
...
rpc("eth_getBlockByHash") do(data: array[32, byte], fullTransactions: bool) -> BlockObject:
...
rpc("eth_getBlockByNumber") do(quantityTag: string, fullTransactions: bool) -> BlockObject:
...
rpc("eth_getTransactionByHash") do(data: Uint256) -> TransactionObject:
...
rpc("eth_getTransactionByBlockHashAndIndex") do(data: UInt256, quantity: int) -> TransactionObject:
...
rpc("eth_getTransactionByBlockNumberAndIndex") do(quantityTag: string, quantity: int) -> TransactionObject:
...
rpc("eth_getTransactionReceipt") do(data: UInt256) -> ReceiptObject:
...
rpc("eth_getUncleByBlockHashAndIndex") do(data: UInt256, quantity: int64) -> BlockObject:
...
rpc("eth_getUncleByBlockNumberAndIndex") do(quantityTag: string, quantity: int64) -> BlockObject:
...
rpc("eth_getCompilers") do() -> seq[string]:
  ## Returns a list of available compilers in the client.
  ##
  ## Returns a list of available compilers.
  result = @[]

rpc("eth_compileSolidity") do(sourceCode: string) -> seq[byte]:
...
rpc("eth_compileLLL") do(sourceCode: string) -> seq[byte]:
...
rpc("eth_compileSerpent") do(sourceCode: string) -> seq[byte]:
...
rpc("eth_newFilter") do(filterOptions: FilterOptions) -> int:
...
rpc("eth_newPendingTransactionFilter") do() -> int:
  ## Creates a filter in the node, to notify when a new block arrives.
  ## To check if the state has changed, call eth_getFilterChanges.
  ##
  ## Returns integer filter id.
...
rpc("eth_uninstallFilter") do(filterId: int) -> bool:
...
rpc("eth_getFilterChanges") do(filterId: int) -> seq[LogObject]:
  ## Polling method for a filter, which returns an list of logs which occurred since last poll.
  ##
  ## filterId: the filter id.
  result = @[]

rpc("eth_getFilterLogs") do(filterId: int) -> seq[LogObject]:
  ## filterId: the filter id.
...
rpc("eth_getWork") do() -> seq[UInt256]:
...
rpc("eth_submitWork") do(nonce: int64, powHash: Uint256, mixDigest: Uint256) -> bool:
...
rpc("eth_submitHashrate") do(hashRate: UInt256, id: Uint256) -> bool:
...
rpc("shh_version") do() -> string:
  ## Returns string of the current whisper protocol version.
  discard

rpc("shh_post") do(message: WhisperPost) -> bool:
...
rpc("shh_newIdentity") do() -> array[60, byte]:
  ## Creates new whisper identity in the client.
  ##
  ## Returns the address of the new identiy.
  discard

rpc("shh_hasIdentity") do(identity: array[60, byte]) -> bool:
...
rpc("shh_newFilter") do(filterOptions: FilterOptions, to: array[60, byte], topics: seq[UInt256]) -> int: # TODO: Is topic of right type?
...
rpc("shh_uninstallFilter") do(id: int) -> bool:
...
rpc("shh_getFilterChanges") do(id: int) -> seq[WhisperMessage]:
...
rpc("shh_getMessages") do(id: int) -> seq[WhisperMessage]:
